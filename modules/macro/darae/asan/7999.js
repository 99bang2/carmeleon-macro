            ...
            while(checkArray.length > 0){
                ...
                if(element){
                    ...
                    if( evalData === carNumber|| ( evalData.length > 4 && carNumber.includes(evalData))){
                        ...
                        isCarNumber=true
                        await page.waitForTimeout(1000)
                        for(let count = 0 ; count < repeatCount[discountTicketUid] ; count++) {
                        ...
                        let checkCount = isDiscount.split(' ').filter(ele=> ele.includes('[취소]')).length
                        if(checkCount === repeatCount[discountTicketUid]) {
                            result = true
